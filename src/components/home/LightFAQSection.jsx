      ...
      <div className={`w-full h-full rounded-[15px] overflow-hidden transition-all duration-300 relative z-10 ${
        ...
        >
          <span className={`font-black text-base md:text-lg transition-colors pr-4 uppercase tracking-wide ${
            isOpen ? 'text-[#16C784]' : 'text-gray-900 group-hover:text-[#16C784]'
          }`}>
            {index + 1}. {faq.q}
          </span>
          <span className={`shrink-0 p-2 rounded-full border transition-colors ${
            isOpen ? 'bg-[#16C784]/10 border-[#16C784] text-[#16C784]' : 'bg-black/5 border-black/10 text-gray-500 group-hover:text-[#16C784] group-hover:bg-[#16C784]/10 group-hover:border-[#16C784]/30'
          }`}>
        ...
        >
          <div className="p-6 lg:p-7 text-gray-600 text-sm lg:text-[15px] leading-relaxed font-semibold">
            {faq.a}
          </div>
        </div>
      </div>
...
const LightFAQSection = () => {
  ...
  return (
    <section ref={containerRef} className="relative pt-0 pb-0 lg:pt-16 lg:pb-0 px-4 sm:px-6 md:px-20 overflow-hidden bg-[#FCFDFD] border-t border-black/5 flex flex-col justify-between" id="faq">
...
export default LightFAQSection;
