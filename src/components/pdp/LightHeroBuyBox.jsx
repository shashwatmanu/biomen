...
const LightHeroBuyBox = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (buybox) {
        ...
        if (ctaBtn) {
          pastHero = ctaBtn.getBoundingClientRect().bottom < 0;
        } else {
  ...
  return (
    <>
      <section
        id="buybox"
        className={`pt-[85px] lg:pt-[140px] pb-12 lg:pb-24 px-6 md:px-20 bg-[#FCFDFD] relative overflow-hidden flex items-center ${isStickyVisible ? 'z-[60]' : 'z-10'}`}
      >
        {/* Background glow effects */}
        <div className="absolute top-0 left-1/4 w-[800px] h-[800px] bg-[#E0F2EB]/20 rounded-full blur-[120px] pointer-events-none mix-blend-screen" />
        ...
        <div className="max-w-7xl mx-auto relative z-10 w-full">
          <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6 lg:gap-16 items-start">

            {/* Left: Product Image Gallery (Takes up 5 cols) */}
            <div className="flex flex-col lg:col-span-5 w-full gap-6">
              ...
              <div className=" w-full flex flex-col gap-6">
                ...
                <div className="relative w-full">
                  {/* Main Showcase Card */}
                  <div
                    className="aspect-square bg-gradient-to-b from-white/5 to-transparent rounded-[2rem] md:rounded-[3rem] overflow-hidden relative flex flex-col items-center justify-center border border-black/10 shadow-2xl p-0 group bg-white select-none w-[78%] md:w-full max-w-[480px] md:mx-auto"
                  >
                    ...
                    <button
                      ...
                      className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/80 border border-black/10 flex items-center justify-center text-gray-900 hover:bg-white hover:border-black/30 z-30 transition-all opacity-100 md:opacity-0 md:group-hover:opacity-100 cursor-pointer"
                    >
                      &larr;
                    </button>
                    <button
                      onClick={() => scrollToIdx(activeIdx === images.length - 1 ? 0 : activeIdx + 1)}
                      className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/80 border border-black/10 flex items-center justify-center text-gray-900 hover:bg-white hover:border-black/30 z-30 transition-all opacity-100 md:opacity-0 md:group-hover:opacity-100 cursor-pointer"
                    >
                      &rarr;
                    </button>
                  </div>

                  {/* Mobile Vertical Thumbnails (Hidden on Desktop) */}
                  <div className="absolute top-0 right-0 h-full w-[19%] md:hidden flex flex-col gap-2 overflow-y-auto snap-y snap-mandatory scrollbar-none">
                    {images.map((img, idx) => (
                      <button
                        key={img.id}
                        onClick={() => scrollToIdx(idx)}
                        className={`w-full aspect-square rounded-xl border transition-all cursor-pointer overflow-hidden relative flex items-center justify-center bg-black/5 flex-shrink-0 snap-start ${activeIdx === idx ? 'border-[#0FA36B] shadow-lg bg-[#E0F2EB]/20' : 'border-black/10'}`}
                      >
                ...
                {/* Dots indicator row (Moved below card, hidden on mobile) */}
                <div className="hidden md:flex justify-center gap-2 mt-4">
                  {images.map((img, idx) => (
                    <button
                      key={img.id}
                      onClick={() => scrollToIdx(idx)}
                      className={`w-2.5 h-2.5 rounded-full transition-all duration-300 cursor-pointer ${activeIdx === idx ? 'bg-[#16C784] w-6' : 'bg-white/15 hover:bg-black/50'}`}
                ...
                <div className="hidden md:grid grid-cols-8 gap-2">
                  {images.map((img, idx) => (
                    <button
                      key={img.id}
                      onClick={() => scrollToIdx(idx)}
                      className={`aspect-square rounded-xl border transition-all cursor-pointer overflow-hidden relative flex items-center justify-center bg-black/5 ${activeIdx === idx ? 'border-[#0FA36B] scale-102 shadow-lg bg-[#E0F2EB]/20' : 'border-black/10 hover:border-[#0FA36B]/50'}`}
                    >
                      <img src={img.url} className={`w-full h-full object-cover ${activeIdx === idx ? 'opacity-100' : 'opacity-60'}`} alt={img.label} />
                    </button>
                  ))}
            ...
            <div className="flex flex-col gap-5 lg:gap-0 lg:space-y-6 lg:col-span-7 lg:justify-center text-left w-full">
              ...
              <div className="flex flex-col w-full space-y-1">
                ...
                <div className="text-lg md:text-xl font-black tracking-widest flex items-center gap-2 uppercase">
                  ...
                  <span className="text-gray-900">BIOMEN <span className="text-[#16C784]">LABS</span></span>
                </div>
                {/* Stars & Social Proof */}
  ...
  FOUNDER BATCH RELEASE NOW LIVE
              ...
              <div className="hidden lg:flex w-full flex-col space-y-4 pt-4 lg:pt-0">
                ...
                <p className="text-base text-gray-600 leading-relaxed font-medium">
                  T-CORE is a premium, clinical-grade herbal blend made for everyday energy, healthy testosterone support, recovery, and performance. Five carefully chosen herbal extracts, thoughtfully formulated to work as part of your daily routine. No fillers. Nothing unnecessary.
              ...
              <div className="hidden">
                <div className="bg-white border border-[#0FA36B]/20 p-1 rounded-full flex gap-1 shadow-2xl relative">
                  <button
                    onClick={() => setIsSubscription(false)}
                    className={`px-3 sm:px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-wider transition-all duration-300 ${!isSubscription ? 'bg-[#0FA36B] text-gray-800 shadow-lg' : 'text-gray-600 hover:text-gray-900'}`}
                  >
                    One-Time Purchase
                  </button>
                  <button
                    ...
                    className={`px-3 sm:px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-wider transition-all duration-300 flex items-center gap-1 ${isSubscription ? 'bg-[#D85A1F] text-gray-800 shadow-lg' : 'text-gray-600 hover:text-gray-900'}`}
                  >
                    Subscribe & Save
                    <span className="bg-white/30 text-[8px] text-[#7FE7B3] px-1.5 py-0.5 rounded border border-black/5 font-black uppercase shrink-0">-15%</span>
                  </button>
                </div>
              </div>

              {/* Premium Selector Deck */}
              <div className="pt-0 lg:pt-4 mt-0 lg:mt-0 lg:border-t lg:border-black/10 space-y-3 lg:space-y-4 w-full">
                <h3 className="text-[10px] lg:text-xs font-black uppercase tracking-[0.2em] text-[#BFA46A] mb-1">
                    ...
                    .map((bundle, index) => {
                      ...
                      return (
                        <div key={bundle.id} className="relative h-full flex flex-col">
                          {bundle.best && (
                            <span className="absolute -top-2.5 right-4 bg-[#0FA36B] text-gray-900 text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full shadow-lg z-30">
                              BEST RESET VALUE
                            </span>
                          )}
                          ...
                          <div
                            ...
                            className={`relative p-[1.5px] rounded-2xl overflow-hidden transition-all duration-500 cursor-pointer flex-1 flex flex-col ${isSelected
                                ? 'border-transparent bg-[#0FA36B] ring-2 ring-[#0FA36B]/25 scale-[1.02] z-10 shadow-2xl shadow-[#0FA36B]/15'
                                : 'bg-black/5 hover:scale-[1.01] border-transparent'
                              } group/spotlight`}
                            style={{
                          ...
                          >
                            ...
                            <div className={`w-full h-full rounded-[14px] p-5 flex flex-col justify-between text-left relative z-10 transition-colors duration-500 overflow-hidden flex-1 ${isSelected ? 'bg-gradient-to-br from-[#E0F2EB]/90 to-white/95' : 'bg-white/60'
                              }`}>
                              ...
                              <div className="absolute inset-0 z-0 opacity-100 pointer-events-none group-hover/spotlight:opacity-100 transition-opacity duration-700">
                                <Canvas camera={{ position: [0, 0, 7], fov: 40 }} dpr={[1, 2]} gl={{ antialias: false, alpha: true }}>
                                  <React.Suspense fallback={null}>
                                    ...
                                    <ScienceProductModel scale={[1, 1, 1]} position={[2.5, -0.8, 0]} rotation={[0, -0.65, -0.05]} />
                              ...
                              <div className="relative z-10">
                                ...
                                <div className="text-base font-black text-gray-900 uppercase mt-1">
                                  {bundle.title}
                                </div>
                                <div className="text-[9px] text-gray-600 mt-1 font-semibold leading-tight">
                                  {bundle.desc}
                                </div>
                              </div>
                              ...
                              <div className="mt-4 flex items-center justify-between w-full relative z-10">
                                <div className="flex flex-col min-h-[44px]">
                                  {isLoadingPrices ? (
                                    <div className="animate-pulse space-y-3 pt-1">
                                      ...
                                      <div className="h-5 w-20 bg-white/15 rounded"></div>
                                    </div>
                                  ) : (
                                    <AnimatedPricing theme="light"
                                      mrp={bundle.mrp}
                                      price={displayPrice}
                                ...
                                {isSelected ? (
                                  <div className="flex items-center gap-2 bg-white/80 rounded-lg p-1 border border-black/10 shadow-inner" onClick={(e) => e.stopPropagation()}>
                                    <button
                                      onClick={() => {
                                    ...
                                    <span className="text-gray-800 text-xs font-black w-4 text-center select-none">
                                      {quantity}
                                    </span>
                                    <button
                                  ...
                                  <div className="w-6 h-6 rounded-full border border-black/20 flex items-center justify-center text-gray-500 transition-colors">
                                    <Plus size={10} />
                                  </div>
                                )}
                              </div>
              ...
              <div className="w-full flex flex-col space-y-4 pt-0">
                {/* Repositioned CTA checkout Button and Stats */}
                <div className="py-0 lg:py-2 flex flex-col sm:flex-row items-center gap-4">
                  <button
                    id="hero-cta-btn"
                    onClick={() => addToCart({
                    ...
                    className="btn-sweep w-full sm:w-auto bg-[#D85A1F] hover:bg-[#b94a17] text-gray-900 py-5 px-16 rounded-full font-black text-xl uppercase tracking-widest transition-all shadow-[0_0_35px_rgba(216,90,31,0.25)] flex items-center justify-center gap-3 hover:scale-[1.02] duration-300 cursor-pointer"
                  >
                  ...
                  <div className="text-left w-full sm:w-auto hidden sm:block">
                    ...
                    <div className="text-xs text-gray-800 font-black uppercase tracking-wider mt-0.5">
                ...
                {/* Trust and Gifts Wrapper */}
                <div className="w-full flex flex-col space-y-6 pt-4 lg:pt-0">
                  ...
                  <div className="hidden bg-[#E0F2EB]/30 border border-[#0FA36B]/20 p-6 rounded-3xl space-y-4">
      ...
      <div id="pdp-sticky-bar" className={`fixed bottom-[calc(14px+env(safe-area-inset-bottom,0px))] left-1/2 -translate-x-1/2 w-[94%] max-w-4xl bg-white/90 backdrop-blur-md border border-black/10 z-[150] pl-4 pr-3 py-2.5 md:px-6 md:py-3.5 rounded-full shadow-[0_15px_45px_rgba(0,0,0,0.1)] flex items-center justify-between transition-all duration-500 ease-out ${isStickyVisible
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex items-center gap-2 sm:gap-3 text-left min-w-0 flex-shrink">
            ...
            <img src={images[0].url} alt="T-CORE" className="w-12 h-12 object-cover bg-black/5 border border-black/10 rounded-xl p-1 hidden md:block" />
            ...
            <div className="h-6 w-px bg-black/5 mx-1.5 shrink-0" />
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0 ml-2">
            ...
            <button
              ...
              className="btn-sweep bg-[#D85A1F] hover:bg-[#b94a17] text-gray-900 py-2.5 px-4 sm:py-3 sm:px-5 md:py-3.5 md:px-8 rounded-full font-black text-[9px] md:text-xs uppercase tracking-wide sm:tracking-widest transition-all shadow-[0_0_20px_rgba(216,90,31,0.2)] hover:scale-[1.02] cursor-pointer text-center whitespace-nowrap"
            >
              Buy Now
            </button>
          </div>
...
export default LightHeroBuyBox;
